                ...
                <div v-else class="selected-file-list">
                  <div class="selected-file-item">
                    <div class="selected-file-item-left">
                      <div class="mini-file-icon">文</div>
                      <div class="selected-file-info">
                        <div class="selected-file-title">{{ tableFillDocumentFile.name }}</div>
                        <div class="selected-file-size">
                          {{ formatFileSize(tableFillDocumentFile.size) }}
                        </div>
                      </div>
                    </div>
                    ...
                    <button
                      class="delete-file-btn"
                      type="button"
                      @click="removeTableFillDocumentFile"
                    >
                      删除
                    </button>
                  </div>
...
const tableFillDocumentFile = ref(null)
...
const canSubmitTableFill = computed(() => {
  ...
  return Boolean(tableFillDocumentFile.value && (selectedTemplateFile.value || activeTemplateMeta.value))
})
...
const clearAllInputs = () => {
  ...
  tableFillDocumentFile.value = null
  commandText.value = ''
}
...
const handleTableFillDocumentChange = (event) => {
  const file = event.target.files?.[0]
  tableFillDocumentFile.value = file || null
  clearResultState()
...
const removeTableFillDocumentFile = () => {
  tableFillDocumentFile.value = null
  clearResultState()
...
const handleTableFillUpload = async () => {
  ...
  if (!tableFillDocumentFile.value) {
    alert('请先上传源文档')
    return
  }
  ...
  formData.append('documents', tableFillDocumentFile.value)
  formData.append('user_request', '')
  ...
  try {
    const fallbackFileName = templateFile.name.replace(/\.(xlsx|xls)$/i, '.docx')
    ...
    userStore.addHistoryRecord({
      fileName: `${templateSourceName} / ${tableFillDocumentFile.value.name}`,
      type: typeNameMap[props.type],
      time: new Date().toLocaleString(),
      status: '已完成',

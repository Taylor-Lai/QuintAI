      ...
      <main class="main-content">
        ...
        <section class="table-card">
          <div class="table-head">
            ...
            <div class="table-tip">共 12{{ total }} 条数据</div> 
          </div>
          <!-- 此处接好后端了，为了匹配后台页面加了静态的12 -->
          ...
          <div class="pagination">
            ...
            <span class="page-text">
              第 {{ queryForm.pageNum }} / 2{{ totalPages }} 页
            </span>
            <!-- 此处接好后端了，为了匹配后台页面加了静态的2 -->
...
const filterUsers = (list) => {
  ...
  if (queryForm.accountStatus) {
    result = result.filter(
      (item) => normalizeAccountStatus(item.accountStatus) === queryForm.accountStatus
    )
  }

  if (queryForm.loginStatus) {
    result = result.filter(
      (item) => normalizeLoginStatus(item.loginStatus) === queryForm.loginStatus
    )
  }
...
const getUserPage = async () => {
  ...
  try {
    const params = {
      ...
      accountStatus: queryForm.accountStatus,
      loginStatus: queryForm.loginStatus,
    ...
    const filteredList = filterUsers(normalizedList)
    ...
    if (!queryForm.keyword && !queryForm.accountStatus && !queryForm.loginStatus && backendTotal) {

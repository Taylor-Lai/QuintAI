...
允许的操作只有：`filter`、`exclude`、`group_by`、`aggregate`、`sort`、`limit`、`impute`、`normalize_unit`、`join`、`derive`、`project`、`deduplicate`、`rank`、`pivot`、`unpivot`、`window`。

每个 operation 必须包含：

- `operation_id`：唯一标识；
- `op`：操作类型；
- `target_table_id`：操作所属目标表；模板包含多个目标表时必须填写；
...
- `join`：必须有两个输入数据集；同名键用 `on`，不同名键同时使用 `left_on` 和 `right_on`；已知关联基数时用 `validate` 指定 `one_to_one`、`one_to_many` 或 `many_to_one`；
